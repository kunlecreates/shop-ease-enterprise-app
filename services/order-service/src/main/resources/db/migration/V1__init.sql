-- order-service schema (Microsoft SQL Server)
-- Run on the target database. Creates schema [order_svc] if missing.
...
IF NOT EXISTS (
  SELECT 1 FROM sys.indexes WHERE name = 'ix_carts_user_ref' AND object_id = OBJECT_ID('order_svc.carts')
)
...
IF NOT EXISTS (
  SELECT 1 FROM sys.indexes WHERE name = 'ix_orders_user_ref' AND object_id = OBJECT_ID('order_svc.orders')
)
...
IF NOT EXISTS (
  SELECT 1 FROM sys.indexes WHERE name = 'ix_payments_order_id' AND object_id = OBJECT_ID('order_svc.payments')
)
...
IF NOT EXISTS (
  SELECT 1 FROM sys.indexes WHERE name = 'ix_payment_tx_payment_id' AND object_id = OBJECT_ID('order_svc.payment_transactions')
)
...
IF NOT EXISTS (
  SELECT 1 FROM sys.indexes WHERE name = 'ix_order_events_order_id' AND object_id = OBJECT_ID('order_svc.order_events')
)
