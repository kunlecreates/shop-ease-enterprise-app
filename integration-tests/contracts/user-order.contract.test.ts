...
maybe('User -> Order contract: create cart, add item, place order', async () => {
  ...
  let respCreate;
  try {
    respCreate = await request('post', '/api/carts', { user_ref: 'test-user' });
  } catch (e) {
  ...
  // Add item
  const addItem = await request('post', `/api/carts/${cartId}/items`, { product_ref: 'prod-1', quantity: 1 }).catch(() => ({ status: 500 }));
